...
class RCModel(object):
    ...
    def __init__(self, vocab, args):
        ...
        #self.saver_every_epoch = tf.train.Saver()
    ...
    def _train_epoch(self, data, indices, dropout_keep_prob,batch_size, save_dir, save_prefix, epoch):
        """
        ...
        test_every_n_batch = 4000
        save_every_n_batch = 500
        total_num, total_loss = 0, 0
        log_every_n_batch, n_batch_loss = 50, 0
        for bitx, batch_num in enumerate(indices, 1):
            batch_file = os.path.join('../data/batch', str(batch_num) + '.json')
            #print('************'+str(batch_num))
            with open(batch_file, 'r') as fin:
                batch = json.loads(fin.readline().strip())
            ...
            if log_every_n_batch > 0 and bitx % log_every_n_batch == 0:
                ...
                n_batch_loss = 0
            if test_every_n_batch > 0 and bitx % test_every_n_batch == 0:
                self.logger.info('eval from batch {} to {}'.format(
                    bitx - test_every_n_batch + 1, bitx))
                #eval_batches = data.gen_mini_batches('dev', batch_size, pad_id, shuffle=False)
                #eval_loss, bleu_rouge = self.evaluate(eval_batches)
                #self.logger.info('Dev eval loss {}'.format(eval_loss))
                #self.logger.info('Dev eval result: {}'.format(bleu_rouge))
            if save_every_n_batch > 0 and bitx % save_every_n_batch == 0:
                self.save(save_dir, save_prefix+'_'+str(epoch)+'_'+str(bitx))

        return 1.0 * total_loss / total_num

    def train(self, data, epochs, batch_size, save_dir, save_prefix,
              dropout_keep_prob, evaluate=False):
        """
        ...
        #for bitx, batch in enumerate(train_batches, 1):
            #batch_file = os.path.join('../data/batch', str(bitx-1) + '.json')
            #with open(batch_file, 'w') as fout:
                #fout.write(json.dumps(batch, ensure_ascii=False) + '\n')
                #total_batch = bitx
        #del train_batches
        total_batch = 8000
        del data.train_set
        ...
        for epoch in range(1, epochs + 1):
            ...
            train_loss = self._train_epoch(data, indices, dropout_keep_prob, batch_size, save_dir, save_prefix, epoch)
    ...
    def evaluate(self, eval_batches, result_dir=None, result_prefix=None, save_full_info=False, save=True):
        """
        ...
        if not save: return pred_answers
    ...
    def save_every_epoch(self, model_dir, model_prefix):
        """
        Saves the model into model_dir with model_prefix as the model indicator
        """
        self.saver_every_epoch.save(self.sess, os.path.join(model_dir, model_prefix))
        self.logger.info('Model saved in {}, with prefix {}.'.format(model_dir, model_prefix))
    ...
    def restore(self, model_dir, model_prefix):
        ...
        self.saver.restore(self.sess, tf.train.latest_checkpoint(os.path.join(model_dir, model_prefix)))
