    ...
    def __init__(self, max_p_num, max_p_len, max_q_len,
        ...
        if test_files:
            for test_file in test_files:
                self.test_set += self._load_dataset(test_file, file=False)
            self.logger.info('Test set size: {} questions.'.format(len(self.test_set)))

    def _load_dataset(self, data_path, train=False, file=True):
        """
        Loads the dataset
        Args:
        ...
        for lidx, line in enumerate(fin):
            sample = json.loads(line.strip()) if file else line
