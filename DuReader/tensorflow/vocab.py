...
class Vocab(object):
    ...
    def load_pretrained_embeddings(self, embedding_path):
        ...
        """  
        trained_embeddings = {}
        pretrained_embeddings = np.load('../data/embedding/word_emb.npy')
        with open(embedding_path, 'r') as fin:
            for line in fin:
                ...
                if contents[1][0] == ('t'):
                    index_embedding = int(contents[0])
                else:
                    index_embedding = int(contents[1])
                #pretrained_embeddings = np.load('../data/embedding/word_emb.npy')
                #print(contents[1])
                if token not in self.token2id:
                    continue
                #trained_embeddings[token] = list(map(float, contents[1:]))
                trained_embeddings[token] = pretrained_embeddings[index_embedding]
